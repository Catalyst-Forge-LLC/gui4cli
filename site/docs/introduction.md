...
The product is a local CLI (`gui4cli` on npm). This site explains it. The window is a [windowd](https://www.npmjs.com/package/windowd) app (NW.js + Vite), not a browser tab.
...
A typical Commander or yargs script works. JSDoc detect is first-pass. `--help` fallback is thin. File fields are text paths. The first windowd launch downloads ~200 MB of NW.js, so a cold machine is slower than a cached one.
