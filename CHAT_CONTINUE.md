...
This chat was about to die because the folder/repo is being renamed. **Read this file first**, then `docs/PHASE_1_BRIEF.md` and `.forgetrail/workflow_tracking.json`. Do not re-litigate Phase 1 unless the user asks.
...
1. ForgeTrail greenfield kickoff. Genesis folded into `docs/GENESIS.md`.
...
4. `.forgetrail/workflow_tracking.json` (decisions D1–D10, gotchas)
...
Not a web CRUD app. Remap ForgeTrail “CRUD / pnpm dev” to:

`npx gui4cli fixtures/<commander-or-yargs-script>` → form → Run → live output → exit code.

Then: `CONTEXT_PROMPT.md` (merge the brief), `README.md`, `TODO.md`, `.forgetrail/IDEAS.md`. Do not dump extra ForgeTrail templates.
